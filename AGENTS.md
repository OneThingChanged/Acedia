...
- Acedia 자체 계정 분산 요청은 loopback API 포트 `3020`을 사용한다. Dashboard는 기존 포트를 유지한다.

- 독립 Acedia Usage Server는 `3007`을 사용한다. 개발 PC의 `3004`~`3006`은 기존 서비스가 사용하므로 중복 실행하지 않는다. 데스크톱 개발 서버 `4420`과 별개다.
