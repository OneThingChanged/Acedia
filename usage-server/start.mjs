...
const service = createUsageServer({ databasePath: path.join(home, 'server.db'), adminToken, localTestNoLogin, clientPackagePath: path.join(path.dirname(fileURLToPath(import.meta.url)), 'downloads/acedia-usage-client.zip'), allowLocalAdmin: adminToken === 'admin' && ['127.0.0.1', '::1'].includes(host) });
