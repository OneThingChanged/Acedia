...
async function exercise() {
  ...
  localStorage.setItem('multiagent.statusBar.v1', JSON.stringify({selectedAccount:'codex'}));
  ...
  check(segments.length===1, 'Legacy selection was not preserved');
  ...
  const choice = key => panel().querySelector('input[type="checkbox"][value="'+key+'"]');
  const currentKeys = () => [...document.querySelectorAll('.usage-status-provider[data-profile-key]')].map(el=>el.dataset.profileKey);
  const currentKey = () => currentKeys()[0];
  choice('claude:work').click(); await wait();
  check(currentKeys().includes('codex') && currentKeys().includes('claude:work') && document.querySelector('.usage-status-provider[data-profile-key="claude:work"]').textContent.includes('한도 확인 전'),'Second selection replaced first or pending quota fabricated');
  check(panel().querySelectorAll('input[type="checkbox"]:checked').length===2,'Multiple accounts were not checked');
  ...
  try {
    ...
    check(currentKeys().length===2 && !currentKeys().includes('codex:personal') && document.querySelector('[role="status"]').textContent.includes('저장하지 못했습니다'),'Failed status selection was published');
  } finally { Storage.prototype.setItem = originalSet; }
  choice('codex:personal').click(); await wait(); check(currentKeys().length===3 && currentKeys().includes('codex:personal'),'Third selection did not update status bar');
  window.fixtureShow('closed'); await wait(); window.fixtureShow('usage'); await wait(); await wait(); await wait();
  check(currentKeys().length===3 && currentKeys().includes('codex:personal'),'Selections lost when reopening workspace');
  ...
  changeFromPeer({selectedAccounts:['claude:work']}); await wait(); check(currentKey()==='claude:work','Selection did not sync from another window');
  ...
  await wait(); await wait(); await wait(); check(currentKeys().length===0,'Removed selected account remained visible');
  ...
  check([...panel().querySelectorAll('input[type="checkbox"]')].every(input=>input.disabled),'Disabled providers could be selected');
  changeFromPeer({codex:true,claude:true}); await wait();
  choice('claude:work').click(); await wait(); check(currentKeys().length===0,'Last checkbox did not clear selection');
  window.fixtureShow('closed'); await wait(); window.fixtureShow('usage'); await wait(); await wait();
  check(currentKeys().length===0,'Explicit empty selection reset on reopen');
  await click('에이전트 사용량'); choice('codex').click(); choice('claude').click(); await wait();
  check(currentKeys().length===2,'Could not reselect multiple accounts after clearing');
  ...
  // A slow live refresh must survive focus/poll reads and cannot be queued twice.
  let finishRefresh;
  window.fixtureRefreshHandler = () => new Promise(resolve => { finishRefresh = resolve; });
  const beforeRefresh = window.fixtureCalls.filter(call => call.command === 'usage_rate_limits_get' && call.args.refresh).length;
  await click('모든 계정 새로고침');
  check(button('갱신 중…').disabled && document.querySelector('.usage-status-refresh').disabled, 'Refresh buttons allow overlapping requests');
  check(panel().textContent.includes('최신 한도 조회 중'), 'Per-account refreshing state missing');
  document.querySelector('.usage-status-refresh').click();
  window.dispatchEvent(new Event('focus')); await wait(); await wait();
  check(window.fixtureCalls.filter(call => call.command === 'usage_rate_limits_get' && call.args.refresh).length === beforeRefresh + 1, 'Duplicate live refresh');
  const refreshedProfiles = profiles.map(profile => ({...profile, refresh:{status:profile.key==='codex:personal'?'timeout':profile.key==='claude:work'?'login_required':'success',checkedAt:Date.now()}}));
  const refreshedLimits = refreshedProfiles.filter(profile=>profile.key!=='claude:work').map(profile=>({...base,primary:{...base.primary,usedPercent:profile.key==='codex:personal'?25:68},limitId:profile.id==='default'?profile.provider:profile.key,profile}));
  finishRefresh({updatedAt:Date.now(),profiles:refreshedProfiles,limits:refreshedLimits}); await wait(); await wait();
  check(!button('모든 계정 새로고침').disabled && !document.querySelector('.usage-status-refresh').disabled,'Refresh did not settle');
  const refreshedCard = key => panel().querySelector('[data-profile-key="'+key+'"]');
  check(refreshedCard('codex:work').textContent.includes('68%') && refreshedCard('codex:work').textContent.includes('최신 한도 조회 완료'),'Idle account quota not refreshed');
  check(refreshedCard('codex:personal').textContent.includes('25%') && refreshedCard('codex:personal').textContent.includes('마지막으로 확인한'),'Failed refresh lost cached usage');
  check(refreshedCard('claude:work').textContent.includes('로그인이 필요') && !refreshedCard('claude:work').textContent.includes('%'),'Missing login has fabricated usage');
  geometry();
  window.fixtureRefreshHandler = async () => { throw Error('fixture transport failed'); };
  await click('모든 계정 새로고침');
  check(document.querySelector('.properties-footer').textContent.includes('사용량을 갱신하지 못했습니다'),'Transport failure is not visible in dialog');
  check(refreshedCard('codex:work').textContent.includes('68%'),'Transport failure erased quotas');
  window.fixtureRefreshHandler = undefined;
  await click('닫기'); await click('새로고침');
  check(window.fixtureCalls.at(-1).command==='usage_rate_limits_get' && window.fixtureCalls.at(-1).args.refresh,'Footer refresh did not request live quotas');
  console.log('ACCOUNT_QUOTA_REFRESH_OK '+innerWidth+'x'+innerHeight);
