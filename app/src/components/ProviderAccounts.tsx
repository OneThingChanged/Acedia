...
const stateLabels: Record<string, [string, string]> = {
  default: ["현재 CLI 환경 사용", "Use current CLI environment"], empty: ["로그인 필요", "Login required"], pending: ["브라우저에서 로그인 중…", "Signing in through browser…"],
...
export function AccountSelect({ value, onChange, disabled = false, label, hint, provider = "codex" }: {
  provider?: "codex" | "claude"; value?: string; onChange: (id: string) => void; disabled?: boolean; label?: string; hint?: string;
}) {
  ...
  useEffect(() => {
    ...
    const poll = () => invoke<Account[]>(`${provider}_accounts_list`)
      .then((list) => { if (live) { setAccounts(list); setError(""); } })
...
export function AccountsPanel({ defaultAccountId = "default", provider = "codex" }: { defaultAccountId?: string; provider?: "codex" | "claude" }) {
  const { text } = useAppLanguage();
  const [accounts, setAccounts] = useState<Account[]>([]);
  ...
  const refresh = async () => setAccounts(await invoke<Account[]>(`${provider}_accounts_list`));
  useEffect(() => {
    let live = true;
    setAccounts([]); setError("");
    const poll = () => invoke<Account[]>(`${provider}_accounts_list`)
  ...
  return <section className="codex-account-management" data-account-provider={provider}>
    ...
    <p className="check-hint">{text("계정별로 한 번 로그인한 후 새 세션이나 세션의 실행 옵션에서 선택하세요. 서로 다른 계정의 세션을 동시에 실행할 수 있습니다. 계정별 인증·설정·기록은 별도로 저장됩니다. 로컬 세션에서 지원합니다.", "Sign in once per account, then select it for a new session or in session launch options. Sessions with different accounts can run at the same time. Each account has separate credentials, settings, and history. Available for local sessions.")}</p>
    ...
    {accounts.map((a) => <div className="agent-settings-row" key={a.id}>
      ...
      {a.id !== "default" && (a.state === "pending"
        ? <button className="btn-secondary" disabled={busy} onClick={() => void run(() => invoke(`${provider}_accounts_cancel_login`))}>{text("취소", "Cancel")}</button>
        : <button className="btn-secondary" disabled={busy || pending} onClick={() => void run(() => invoke(`${provider}_accounts_login`, { accountId: a.id }))}>{text("브라우저 로그인", "Browser login")}</button>)}
    </div>)}
    {adding && <div className="agent-settings-row agent-account-add">
      <input autoFocus aria-label={(provider === "codex" ? text("Codex 계정 이름", "Codex account name") : text("Claude 계정 이름", "Claude account name"))} placeholder={text("계정 이름 (예: 개인, 업무)", "Account name (e.g. personal, work)")} maxLength={80} value={label} onChange={(e) => setLabel(e.target.value)} />
      <button className="btn-secondary" disabled={busy || !label.trim()} onClick={() => void run(async () => {
        await invoke(`${provider}_accounts_create`, { label: label.trim() }); setLabel(""); setAdding(false);
