...
export function StatusBarSettingsPanel({enabled, onEnabledChange}: {enabled:boolean; onEnabledChange:(value:boolean)=>void}) {
  ...
  return <section className="app-settings-section">
    <p className="check-hint">{text('변경하면 모든 앱 작업창에 바로 적용합니다. 제공자 항목은 받은 계정 한도만 표시하며 로컬 토큰 집계를 한도 비율로 환산하지 않습니다.', 'Changes apply immediately across app workspaces. Provider items display reported account limits; local token totals are not converted to quota percentages.')}</p>
